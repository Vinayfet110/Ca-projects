...
void main(){
int k,i,*bc,j,l,loop=9;

long totaladcount=0;
float totalefficiency=0.0,averagecount=0.0,averageefficiency=0.0;
clrscr();
  n=16,alast=n,afirst=0;
//
for(n=16;n<=32;n++){  // loop for 16 to 32
 ...
 for(l=0;l<loop;l++){  //looping
  ...
  //printf("looping n value is :%d",n);
  efficiency=(float)(n+1)/addercount;
  totalefficiency+=efficiency;
  totaladcount+=addercount;
  //printf("\nadder count is :%d",addercount);    long totaladcount=0;
  averagecount=(float)totaladcount/loop;
  averageefficiency=(float)totalefficiency/loop;
 ...
 //averageefficiency=(float)totalefficiency/loop;
   /*
 printf("\ntotal adder count for %d bit is:%ld ",n,totaladcount);
 printf("average counter :%f",averagecount);
 printf("\ntotal efficiency for %d bit is:%f ",n,totalefficiency);
 printf("average efficiency is:%f\n",averageefficiency);
 */
 if(n%2==0)
 printf("\naverage efficiency for %d bit is %f",n,efficiency);
 getch();
